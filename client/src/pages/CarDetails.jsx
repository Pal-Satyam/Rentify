          ...
          className="space-y-6">
            ...
            <div>
              ...
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {[
                  "360 Camera", "Bluetooth", "GPS", "Ventilated Seats", "Rear View Mirror"
                ].map((item) => (
        ...
        onSubmit={handleSubmit} className="shadow-lg h-max sticky top-18 rounded-xl p-6 space-y-6 text-gray-500">
          ...
          <div className='flex flex-col gap-2'>
            ...
            <input value={pickupDate} onChange={(e) => setPickupDate(e.target.value)}
             type="date" className="border border-color px-3 py-2 rounded-lg " required id='pickup-date'
             min={new Date().toISOString().split("T")[0]} />
          </div>
          <div className='flex flex-col gap-2'>
            <label htmlFor='return-date'>Return Date</label>
            <input value={returnDate} onChange={(e) => setReturnDate(e.target.value)}
             type="date" className="border border-color px-3 py-2 rounded-lg " required id='return-date'
             min={new Date().toISOString().split("T")[0]}/>
          </div>
