    ...
    <div className='relative min-h-screen md:flex flex-col items-center pt-8 max-w-13 md:max-w-60 w-full border-r border-borderColor text-sm'>
      ...
      <div className='group relative'>
        ...
        <img src={image ? URL.createObjectURL(image) : user?.image || 
          "https://unsplash.com/photos/silhouette-photo-of-man-on-cliff-during-sunset-_6HzPU9Hyfg"} alt="" 
      ...
      {image && (
        <button className='absolute top-0 right-0 flex pt-2 gap-1 bg-primary/10 text-primary cursor-pointer' 
      ...
      <div className='w-full'>
        {ownerMenuLinks.map((link, index) => (
           ...
           ${link.path === location.pathname ? 'bg-primary/10 text-primary' : 'text-gray-600'}`}>
            ...
            <div className={`${
              link.path === location.pathname && 'bg-primary' } w-1.5 h-8 rounded-1 right-0 absolute `}></div>
           </NavLink> 
        ))}
